...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let error = document.querySelector(".alert-danger"); // Kiểm tra nếu có lỗi
    let success1 = document.querySelector(".alert.alert-success.mt-1"); // Kiểm tra nếu có thông báo thành công
    let success2 = document.querySelector(".alert.alert-success.mt-2"); // Kiểm tra nếu có thông báo thành công
    let hash = window.location.hash; // Lấy fragment từ URL
    ...
    if (error || success2 || hash === "#security") {
        let securityTab = document.querySelector('a[href="#security"]');
        if (securityTab) {
            securityTab.click(); // Kích hoạt tab Security
        }
    }
    if (error || success1 || hash === "#personal") {
        let securityTab = document.querySelector('a[href="#personal]"]');
        if (securityTab) {
            securityTab.click(); // Kích hoạt tab Security
        }
    }
...
function showDialog(action) {
    ...
    title.innerText = action === 'WITHDRAW' ? 'WITHDRAW' : 'TOP_UP';
